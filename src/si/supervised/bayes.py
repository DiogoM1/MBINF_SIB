...
from si.util.train import vectorize_dataset
...
class NaiveBayes(SupervisedModel):
    ...
    def fit(self, dataset):
        ...
        for label in self.labels:
            prob_matrix[label] = [np.unique(dataset.X[dataset.y == label, i], return_counts=True) for i, _ in enumerate(dataset.xnames)]
